...
send "$password\r"
expect "$ "
send "(date \"+%H:%M:%S - %d/%m/%Y\" && sudo checkrestart | grep -E \"Found|distinct\" && echo \"These are the systemd services:\" && sudo checkrestart | grep \"systemctl restart\" && echo \"These are the initd scripts:\" && sudo checkrestart | grep ^\"service\" | grep \"restart\"$) &>> ~/system_updates/.logs/service_checks.log\r"
sleep 1
expect "$ "
